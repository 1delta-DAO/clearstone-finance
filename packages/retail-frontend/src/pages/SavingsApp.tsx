...
export function SavingsApp() {
  ...
  const [kycStatus, setKycStatus] = useState<
    "unknown" | "checking" | "approved" | "not_approved"
  >("unknown");
  const [usdcBalance, setUsdcBalance] = useState<number | null>(null);

  // Check KYC status (whitelist entry exists?)
  useEffect(() => {
    ...
    const [whitelistEntry] = PublicKey.findProgramAddressSync(
      [
        ...
        config.pool.dmMintConfig.toBuffer(),
        publicKey.toBuffer(),
      ],
  ...
  useEffect(() => {
    ...
    connection.getTokenAccountBalance(ata).then((bal) => {
      setUsdcBalance(Number(bal.value.uiAmount));
  ...
  const handleSelfRegister = useCallback(async () => {
    ...
    // Find the user's Civic gateway token PDA
    const CIVIC_PROGRAM = config.programs.civicGateway;
    ...
    <div style={styles.container}>
      ...
      <section style={styles.hero}>
        ...
        <div style={styles.apyBadge}>
          ...
          <span style={styles.apyValue}>~4.2%</span>
        </div>
      </section>
      ...
      <main style={styles.main}>
        {!connected ? (
          ...
          <div style={styles.grid}>
            <PortfolioCard usdcBalance={usdcBalance} config={config} />
            <DepositCard usdcBalance={usdcBalance} config={config} />
          </div>
        )}
      </main>
      ...
      <footer style={styles.footer}>
        ...
        <p style={styles.footerSmall}>
          Powered by Kamino Finance on Solana Devnet
        </p>
      </footer>
    </div>
  );
