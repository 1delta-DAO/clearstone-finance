...
import {
  ...
  borrow,
  repay,
} from "../src/lending/operations.js";
import {
  KLEND_PROGRAM_ID,
  TOKEN_PROGRAM_ID,
...
test("initObligation: data = disc + u64 tag(0) + u8 seed", () => {
  ...
  assert.equal(
    ...
    obligationPda(MARKET, OWNER, 3).toBase58()
  );
});
...
test("initObligation: defaults seed=0 when omitted", () => {
  ...
  assert.equal(
    ix.keys[1].pubkey.toBase58(),
    obligationPda(MARKET, OWNER, 0).toBase58()
  );
});
...
test("deposit: data = depositReserveLiquidity disc + u64 amount", () => {
  const ix = deposit(OWNER, MARKET, RESERVE, MINT, TOKEN_PROGRAM_ID, 1_000_000n);
  assert.equal(ix.data.length, 16);
  assert.deepEqual(
    [...ix.data.subarray(0, 8)],
    [...DISC.depositReserveLiquidity]
  );
  assert.equal(ix.data.readBigUInt64LE(8), 1_000_000n);
});
...
test("deposit: key layout pins obligation, market authority, supply PDAs", () => {
  ...
  assert.equal(
    ...
    marketAuthorityPda(MARKET).toBase58()
  );
  assert.equal(ix.keys[4].pubkey.toBase58(), RESERVE.toBase58());
  // Liquidity supply @ 6, cMint @ 7.
  assert.equal(
    ix.keys[6].pubkey.toBase58(),
    reserveLiquiditySupplyPda(RESERVE, MARKET).toBase58()
  );
  assert.equal(
    ix.keys[7].pubkey.toBase58(),
    reserveCollateralMintPda(RESERVE, MARKET).toBase58()
  );
  // user ATA uses the caller-supplied token program.
  assert.equal(
    ix.keys[9].pubkey.toBase58(),
    ata(MINT, OWNER, TOKEN_PROGRAM_ID).toBase58()
  );
});
...
test("deposit: honours TOKEN_2022 token program for dUSDY-style mints", () => {
  ...
  assert.equal(
    ix.keys[9].pubkey.toBase58(),
    ata(MINT, OWNER, TOKEN_2022_PROGRAM_ID).toBase58()
  );
});
...
test("withdraw: data = disc + u64, 13 accounts (one fewer than deposit — no ATA program)", () => {
  ...
  assert.deepEqual(
    [...ix.data.subarray(0, 8)],
    [...DISC.withdrawObligationCollateral]
  );
  assert.equal(ix.keys.length, 13);
});

test("borrow: data = borrowObligationLiquidity disc + u64 amount; 10-key layout", () => {
  const ix = borrow(OWNER, MARKET, RESERVE, MINT, 500_000n);
  assert.equal(ix.data.length, 16);
  assert.deepEqual(
    [...ix.data.subarray(0, 8)],
    [...DISC.borrowObligationLiquidity]
  );
  assert.equal(ix.data.readBigUInt64LE(8), 500_000n);
  assert.equal(ix.keys.length, 10);
  assert.equal(
    ix.keys[7].pubkey.toBase58(),
    reserveFeeVaultPda(RESERVE, MARKET).toBase58()
  );
});

test("repay: data = repayObligationLiquidity disc + u64, 8-key layout (no cMint)", () => {
  const ix = repay(OWNER, MARKET, RESERVE, MINT, 250_000n);
  assert.equal(ix.data.length, 16);
  assert.deepEqual(
    [...ix.data.subarray(0, 8)],
    [...DISC.repayObligationLiquidity]
  );
...
test("obligation PDA uses seed consistently across setup + operations", () => {
  ...
  const depositIx = deposit(
    OWNER, MARKET, RESERVE, MINT, TOKEN_PROGRAM_ID, 1n, 5
  );
  // initObligation places obligation at keys[1]; deposit places it at keys[1] too.
  assert.equal(
    initIx.keys[1].pubkey.toBase58(),
    depositIx.keys[1].pubkey.toBase58()
  );
});
