...
export function decideDelegatedRoll(vault, delegation, nowTs, nowSlot, graceSec) {
    ...
    const floor = fixedYield.delegation.slippageFloor(deployed, delegation.maxSlippageBps);
    return {
        reason: "ready",
 ...
 */
async function deriveCrankAccountsFor(conn, vault, marketPk) {
    ...
    const mintSy = new PublicKey(data.slice(8 + 32 + 2 + 1 + 64, 8 + 32 + 2 + 1 + 96));
...
export async function executeDelegatedRoll(conn, cfg, vault, delegation, decision) {
    ...
    const vaultPk = new PublicKey(vault.vault);
    const preIxs = [
        createAssociatedTokenAccountIdempotentInstruction(cfg.curatorKeypair.publicKey, toSide.from.vaultPtAta, vaultPk, toSide.from.mintPt, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID),
        createAssociatedTokenAccountIdempotentInstruction(cfg.curatorKeypair.publicKey, toSide.from.vaultLpAta, vaultPk, toSide.from.mintLp, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID),
    ];
...
//# sourceMappingURL=roll-delegated.js.map