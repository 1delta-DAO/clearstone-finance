...
async function tick() {
    ...
    const skipCuratorFallback = process.env.SKIP_CURATOR_FALLBACK === "1";
    log({
        event: "tick.start",
    ...
    for (const v of vaults) {
        ...
        const delegations = filterLive(delegationsByVault.get(v.vault) ?? [], nowSlot);
        for (const d of delegations) {
            const decision = decideDelegatedRoll(v, d, nowTs, nowSlot, cfg.maturityGraceSec);
