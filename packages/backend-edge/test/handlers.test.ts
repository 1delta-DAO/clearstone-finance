...
import type { CuratorVaultDto } from "../src/fixed-yield.js";
...
async function GET(path: string, env: Env = EMPTY_ENV): Promise<Response> {
...
test("GET /fixed-yield/markets/:id returns that market on a known id", async () => {
  ...
  assert.equal(res.status, 200);
  const body = (await res.json()) as { market: { id: string } };
...
test("GET /fixed-yield/vaults groups markets under a vault envelope", async () => {
  ...
  const body = (await res.json()) as {
    vaults: Array<{
      id: string;
      markets: Array<{ maturityTs: number }>;
      underlying: string;
      baseDecimals: number;
    }>;
  };
  ...
  for (const v of body.vaults) {
    ...
    for (let i = 1; i < v.markets.length; i++) {
      assert.ok(
        v.markets[i].maturityTs >= v.markets[i - 1].maturityTs,
        "markets must be sorted by maturityTs ascending"
      );
    }
  }
});
...
test("GET /fixed-yield/vaults/:id/positions/:user returns empty position when registry is unset", async () => {
  const res = await GET(
    "/fixed-yield/vaults/11111111111111111111111111111111/positions/DoPdypbndDDtjhxQ1P3WozCmHw4rWSF5bzABeBX2VToA"
  );
  assert.equal(res.status, 200);
  const body = (await res.json()) as {
    position: {
...
test("GET /fixed-yield/curator-vaults/:id/positions/:user returns empty on an invalid vault pubkey (no RPC)", async () => {
  ...
  const res = await GET(
    "/fixed-yield/curator-vaults/not-a-valid-pubkey/positions/also-bad"
  );
  assert.equal(res.status, 200);
  const body = (await res.json()) as {
    position: {
...
} as const satisfies CuratorVaultDto;
...
test("contract: keeper's canonical DTO literal satisfies CuratorVaultDto on the edge side", () => {
  ...
  for (const key of [
    ...
    assert.ok(
      key in KEEPER_CONTRACT_FIXTURE,
      `edge must emit ${key} in CuratorVaultDto`
    );
  }
});
